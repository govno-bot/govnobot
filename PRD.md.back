...
## Tasks
- [x] Define user stories and acceptance criteria for all bot interactions (TDD/BDD)
- [x] Implement message receiving and command parsing with failing tests first (TDD)
- [x] Develop user feedback and error messaging flows, writing tests before code
- [x] Add conversation history and settings management with BDD-style scenarios
 - [x] Test and refine all user interaction paths until all acceptance tests pass
- [x] Document user interaction flows and testing approach for contributors
- [x] Read TODO.md and PROGRESS.md, add more tasks to this list.
...
- [x] Run all tests. Read output carefuly. Notice some tests are not passing. Fix.
...
- [x] Fix heap out of memory bug
- [x] Complete Documentation Phase (ARCHITECTURE.md, API.md, TEST-STRATEGY.md, etc.)
- [x] Use govnobod to send message to admin. Make sure message send. Env vars: TELEGRAM_GOVNOBOT_TOKEN, TELEGRAM_GOVNOBOT_ADMIN_CHATID and TELEGRAM_GOVNOBOT_ADMIN_USERNAME
...
2. **BDD/Acceptance Tests:**
	 - Use BDD-style descriptions and scenarios for user flows (see `test/unit/message-handler.test.js`).
...
5. **Documentation:**
	 - Update this section if new user flows or testing practices are introduced.

### Example: Adding a New Command

1. Add a failing test for the new command in `test/unit/message-handler.test.js`.
...
- Node.js only: absolutely no external npm packages or dependencies
- All features and modules must be covered by automated tests (TDD-first)
- Directory structure includes `src/`, `test/`, `data/`, and comprehensive documentation
- Project is cross-platform (Windows, macOS, Linux) and educational in design
- Documentation files guide new users, provide quick reference, and track progress
