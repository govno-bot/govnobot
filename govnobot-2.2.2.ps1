...
# Admin users (username -> chat_id)
$script:AdminUsers = @{
    "YourTelegramUserNameWithoutAtSign" = 696969300
}
...
function RestrictToAdmin {
    ...
    if ($script:AdminUsers.ContainsKey($username)) {
        if ($script:AdminUsers[$username] -eq $chatId) {
...
function Invoke-AI-Fallback {
    ...
    if ($openAiKey) {
        try {
            Write-Log "Attempting OpenAI API..."
            $headers = @{
            ...
            return @{ success = $true; answer = $answer; source = "OpenAI API" }
        } catch {
            Write-Log "OpenAI API failed: $_" "WARN"
        }
    }
        ...
        "^/ask" {
            ...
            foreach ($entry in $conversationContext[0..($conversationContext.Count-2)]) {
                if ($entry) {
                    ...
                    $contextStr += "`n[$role]: $($entry.content)"
                }
            }
            
            if ($contextStr -and $contextStr.Length -gt 0) {
                $enrichedPrompt = "Context:`n$contextStr`n`nNew question: $question"
            } else {
        ...
        "^/chain" {
            ...
            foreach ($i in 0..($steps.Count - 1)) {
                ...
                $prompt = if ($previousOutput) {
                    "Context from previous step:`n$previousOutput`n`nNext step: $step"
                } else {
                    $step
                }
        ...
        "^/jack" {
            ...
            if ($message -and $message.Length -gt 0) {
                try {
                    ...
                    $resultText = "Message sent. Probbably."
                    if (-not $result) {
